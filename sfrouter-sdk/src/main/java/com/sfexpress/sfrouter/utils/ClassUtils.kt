...
/**
 * 类操作工具, copy from google source code.
...
object ClassUtils {
    ...
    private val isVMMultidexCapable: Boolean
        get() {
            ...
            Log.i(Consts.TAG, "VM with name " + vmName + if (isMultidexCapable) " has multidex support" else " does not have multidex support")
        ...
        for (path in paths) {
            DefaultPoolExecutor.execute {
                ...
                try {
                    ...
                    Log.e("SFRouterCore", "Scan map file in dex files made error.", ignore)
                } finally {
                    if (null != dexfile) {
                        try {
                            dexfile.close()
    ...
    fun getSourcePaths(context: Context): List<String> {
        ...
        if (SFRouterManager.debuggable()) { // Search instant run support only debuggable
    ...
    private fun tryLoadInstantRunDexFile(applicationInfo: ApplicationInfo): List<String> {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && null != applicationInfo.splitSourceDirs) {
            ...
            try {
                ...
                Log.e(Consts.TAG, "InstantRun support error, " + e.message)
            }
        }
